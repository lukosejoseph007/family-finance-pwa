			...
			<div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
				<div class="flex-1">
					<div class="mb-4 flex items-center space-x-3">
						...
						<div>
							<h1 class="text-3xl font-bold text-white sm:text-4xl">Financial Reports</h1>
				...
				<div class="flex-shrink-0">
					<div class="rounded-xl border border-white/20 bg-white/10 p-4 backdrop-blur-sm">
						...
						<select
							...
							class="block min-w-[180px] rounded-lg border-gray-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
						>
		...
		<div class="container mx-auto space-y-6 px-4 py-8 pb-12 sm:px-6 lg:px-8">
			...
			{:else}
						...
						<Card class="p-4">
							<div class="flex items-center justify-between">
								<div>
									...
									<p
										class="text-2xl font-bold {monthlyReport.net_amount >= 0
											? 'text-green-600'
											: 'text-red-600'}"
									>
								...
								<div
									class="p-3 {monthlyReport.net_amount >= 0
										? 'bg-green-100'
										: 'bg-red-100'} rounded-full"
								>
									<svg
										class="h-6 w-6 {monthlyReport.net_amount >= 0
											? 'text-green-600'
											: 'text-red-600'}"
										fill="currentColor"
										viewBox="0 0 20 20"
						...
						<Card class="p-4">
							<div class="flex items-center justify-between">
								<div>
									...
									<p
										class="text-2xl font-bold {(monthlyReport.net_amount /
											(monthlyReport.income || 1)) *
											100 >=
										20
											? 'text-green-600'
											: (monthlyReport.net_amount / (monthlyReport.income || 1)) * 100 >= 10
												? 'text-yellow-600'
												: 'text-red-600'}"
									>
				...
				{#if budgetPerformance}
					<Card class="p-6">
						<h2 class="mb-4 text-xl font-semibold text-gray-900">Budget Performance Overview</h2>
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
							<div class="text-center">
								<div class="text-3xl font-bold text-green-600">
									{budgetPerformance.on_track_categories}
								</div>
								<div class="text-sm text-gray-600">On Track</div>
							</div>
							<div class="text-center">
								<div class="text-3xl font-bold text-red-600">
									{budgetPerformance.over_budget_categories}
								</div>
								<div class="text-sm text-gray-600">Over Budget</div>
							</div>
							<div class="text-center">
								<div class="text-3xl font-bold text-yellow-600">
									{budgetPerformance.under_budget_categories}
								</div>
							...
							<div class="text-center">
								<div
									class="text-3xl font-bold {budgetPerformance.performance_score >= 80
										? 'text-green-600'
										: budgetPerformance.performance_score >= 60
											? 'text-yellow-600'
											: 'text-red-600'}"
								>
				...
				{#if categorySpending.length > 0}
					<Card class="p-6">
						<h2 class="mb-4 text-xl font-semibold text-gray-900">Category Spending Details</h2>
											...
											<td class="px-6 py-4 whitespace-nowrap">
												<div class="flex items-center">
													<div class="mr-2 h-2 flex-1 rounded-full bg-gray-200">
														<div
															class="h-2 rounded-full {category.percentage_used > 100
																? 'bg-red-500'
													...
													<span
														class="text-sm font-medium {category.percentage_used > 100
															? 'text-red-600'
															: category.percentage_used > 80
																? 'text-yellow-600'
																: 'text-green-600'}"
