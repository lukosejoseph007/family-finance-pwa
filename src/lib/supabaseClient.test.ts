...
describe('supabaseClient', () => {
  describe('signUp', () => {
    ...
    it('should throw an error if signUp fails', async () => {
      const mockSignUp = vi.fn().mockResolvedValue({ data: null, error: new Error('Sign up failed') });
      vi.mocked(createClient).mockReturnValue({
        auth: {
          signUp: mockSignUp,
        },
  ...
  describe('signIn', () => {
    ...
    it('should throw an error if signIn fails', async () => {
      const mockSignIn = vi.fn().mockResolvedValue({ data: null, error: new Error('Sign in failed') });
      vi.mocked(createClient).mockReturnValue({
        auth: {
          signInWithPassword: mockSignIn,
        },
