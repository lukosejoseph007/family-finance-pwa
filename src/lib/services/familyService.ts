...
export async function updateFamilySettings(
	familyId: string,
	updates: { name?: string; settings?: Partial<Family['settings']> }
): Promise<Family> {
...
export async function leaveFamily(): Promise<void> {
	...
	if (currentUser.role === 'admin' && !isLastMember) {
		...
		if (!otherAdmins || otherAdmins.length === 0) {
			throw new Error(
				'You cannot leave the family as the only admin. Please promote another member to admin first.'
			);
		}
...
export async function sendEmailInvitation(
	...
	console.log('📧 Sending email invitation via API:', {
		familyName,
		inviteCode,
		recipientEmail,
		senderName
