-- Complete RLS Fix with Stored Procedure for Family Creation
-- This approach uses a secure stored procedure that bypasses RLS for initial setup
...
DROP POLICY IF EXISTS "Members can manage budgets" ON budgets;

-- Create a secure stored procedure for family creation that bypasses RLS
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'budgets') THEN
        ...
        EXECUTE 'CREATE POLICY "Manage family budgets" ON budgets
            FOR ALL USING (
                family_id IN (
                    SELECT family_id FROM users WHERE id = auth.uid()
                )
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'session') THEN
        ...
        EXECUTE 'CREATE POLICY "Own sessions only" ON session
